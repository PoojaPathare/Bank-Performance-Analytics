DROP TABLE IF EXISTS #Bank_performance;
...
WITH Loan as (
             SELECT 
              SUM(Outstanding_principal)/12 as Avg_Loan_asset,
              ISNULL(SUM(Interest_income),0) as Total_Loan_Income
             FROM Bank_Data..Loans$
), Investment as (
             SELECT 
             SUM(Outstanding_principal)/12 as Avg_Investment_Asset,
             ISNULL(SUM(Interest_income),0) as Total_Investment_Income
             FROM Bank_Data..INVESTMENTS$
),Deposit as (
             SELECT
...
Annual_Non_Interest_Income,
Avg_Shareholders_Equity
)
SELECT 
    (L.Avg_Loan_Asset + I.Avg_Investment_Asset) AS Avg_Earning_Assets,
    (L.Total_Loan_Income + I.Total_Investment_Income) AS Annual_Interest_Income,
    (D.Total_Deposit_Interest_Expense + O.Total_Other_Int_Expense) AS Annual_Interest_Expense,
    (L.Avg_Loan_Asset + I.Avg_Investment_Asset + 
     O.Avg_Cash + O.Avg_SLR + O.Avg_CRR + O.Avg_Other_Asset) AS Avg_Total_Assets,
